...
{  		
  ...
  else if(ch >= 0xD4B1 && ch <= 0xD687)  //Armenian language (UTF-8 encoding)
  {
      if (ch <= 0xD4BF)
      {
          offset = (ch-0xD4B1+208) * (BYTE_HEIGHT * BYTE_WIDTH / 8);
      }
      else if (ch >= 0xD580 && ch <= 0xD5BF)
      {
          offset = (ch-0xD580+0xF+208) * (BYTE_HEIGHT * BYTE_WIDTH / 8);
      }
      else if (ch >= 0xD680 && ch <= 0xD687)
      {
          offset = (ch-0xD680+0x4F+208) * (BYTE_HEIGHT * BYTE_WIDTH / 8);
      }
  }
